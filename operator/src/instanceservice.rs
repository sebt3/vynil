use crate::{Error, Reconciler, Result, ServiceInstance, get_client_name, manager::Context, telemetry};
...
static SERVICE_FINALIZER: &str = "serviceinstances.vynil.solidite.fr";
...
pub async fn reconcile(inst: Arc<ServiceInstance>, ctx: Arc<Context>) -> Result<Action> {
    ...
    let _mes = ctx.metrics.service_instance.count_and_measure(&inst, &trace_id);
    ctx.diagnostics.write().await.last_event = Utc::now();
    let ns = inst.namespace().unwrap_or_default(); // inst is namespace scoped
    let insts: Api<ServiceInstance> = Api::namespaced(ctx.client.clone(), &ns);

    finalizer(&insts, SERVICE_FINALIZER, inst, |event| async {
        match event {
...
impl Reconciler for ServiceInstance {
    ...
    async fn reconcile(&self, ctx: Arc<Context>) -> Result<Action> {
        ...
        let job_name = format!("service--{}--{}", ns, self.name_any());
        ...
        context
            .as_object_mut()
            .unwrap()
            .insert("package_type".to_string(), "service".into());
        context
            .as_object_mut()
            .unwrap()
        ...
        tracing::debug!(
            "Reconcilling ServiceInstance {}/{} Done",
            self.namespace().unwrap(),
            self.name_any()
        );
...
pub fn error_policy(inst: Arc<ServiceInstance>, error: &Error, ctx: Arc<Context>) -> Action {
    ...
    ctx.metrics.service_instance.reconcile_failure(&inst, error);
