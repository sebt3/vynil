use crate::{get_client_name, manager::Context, telemetry, Error, Reconciler, Result, ServiceInstance};
...
pub async fn reconcile(inst: Arc<ServiceInstance>, ctx: Arc<Context>) -> Result<Action> {
    ...
    let _mes = ctx.metrics.service_instance.count_and_measure(&trace_id);
