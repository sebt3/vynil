...
impl Reconciler for TenantInstance {
    ...
    async fn reconcile(&self, ctx: Arc<Context>) -> Result<Action> {
        ...
        context.as_object_mut().unwrap().insert("package_type".to_string(), "tenant".into());
        ...
        } else if ! packages[&self.spec.jukebox].packages.clone().into_iter().any(|p| p.metadata.name == self.spec.package && p.metadata.category == self.spec.category && p.metadata.usage == VynilPackageType::Tenant) {
        ...
        for req in pck.requirements {
            let (res, mes) = req.check_tenant(self, client.clone()).await?;
            if ! res {
