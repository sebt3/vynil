use common::{context::set_service, instanceservice::ServiceInstance, rhaihandler::Script, Result};
use serde::{Deserialize, Serialize};
use clap::Args;

#[derive(Args, Debug, Serialize, Deserialize)]
pub struct Parameters {
    /// Instance namespace to reconfigure
    #[arg(short = 'n', long = "namespace", env = "NAMESPACE", value_name = "NAMESPACE")]
    namespace: String,
    /// Instance name to reconfigure
    ...
    #[arg(
        ...
        default_value = "docker.io/sebt3/vynil-agent:0.4.7"
    )]
    agent_image: String,
    /// version
...
pub async fn run(args: &Parameters) -> Result<()> {
    ...
    let _ = rhai.eval(
        ...
        import(\"reconfigure\") as reconfigure;\n\
        reconfigure::run(instance, context);",
    )?;
    Ok(())
}
