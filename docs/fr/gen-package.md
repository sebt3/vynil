...
flowchart LR
    ...
    CMD --> PRE[hook update_pre]
    PRE --> OCI[Interroge les registries OCI\npour les derniers tags semver]
    OCI --> CMP{Tag plus\nrécent ?}
    CMP -->|oui| UPD[Met à jour package.yaml\nyaml_encode_ordered]
    CMP -->|non| DONE[Aucun changement]
    UPD --> POST[hook update_post]
    POST --> REGEN[Régénère les templates\ngen_system / gen_tenant / gen_service]
```
